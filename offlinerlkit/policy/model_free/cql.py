...
class CQLPolicy(SACPolicy):
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        # with open("/media/YQX/OfflineRL-Kit/cql.txt","a") as f:
        #     f.write(str(conservative_loss1))
        #     f.write("\n")
    ...
    def update_conservative(self,batch: Dict):
        ...
        self.critic1_optim.zero_grad()
        conservative_loss1.backward(retain_graph=True)
        self.critic1_optim.step()

        self.critic2_optim.zero_grad()
        conservative_loss2.backward()
        self.critic2_optim.step()

        self._sync_weight()


    def no_conservative(self, batch: Dict):
        obss, actions, next_obss, rewards, terminals = batch["observations"], batch["actions"], \
                                                       batch["next_observations"], batch["rewards"], batch["terminals"]
        batch_size = obss.shape[0]
